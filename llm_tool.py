...
@tool(
    ...
    description=(
        "Take a screenshot of the current browser page and analyze it with Moondream2. "
        "Returns a description of what's visible on the page. "
        "Use after playwright_navigate to understand what a page looks like without reading HTML."
    ),
    group=PLAYWRIGHT_TOOL_GROUP,
)
def playwright_screenshot() -> dict[str, Any]:
    try:
        ...
        try:
            ...
            from plugins.moondream_vision.runtime import plugin_config_path

            cfg = load_config(plugin_config_path())
            answer = infer_screen_png(png, "Describe what is visible on this webpage in detail. Include any error messages, buttons, forms, or text content.", cfg)
            return {"description": answer}
        except Exception:
            return {"png_base64": png.hex(), "hint": "Moondream 不可用，返回了原始截图数据"}
    except Exception as e:
...
def playwright_evaluate(js_code: str) -> dict[str, Any]:
    ...
    try:
        from plugins.playwright_browser.browser import _ensure_browser
